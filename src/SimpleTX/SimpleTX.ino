...
bool calibrationProcess() {
  ...
  // 15 seconds for moving sticks
    ...
    Serial.print("Move stick full range: Aileron_Min:");
...
void setup (void)
{
  ...
  //analogReference(INTERNAL);  // debug only!


  pinMode(DIGITAL_PIN_SWITCH_ARM, INPUT_PULLUP);

  if (DIGITAL_PIN_SWITCH_AUX2 != 0) {
    pinMode(DIGITAL_PIN_SWITCH_AUX2, INPUT_PULLUP);
  }

  if (DIGITAL_PIN_SWITCH_AUX3a != 0) {
    pinMode(DIGITAL_PIN_SWITCH_AUX3a, INPUT_PULLUP);
    pinMode(DIGITAL_PIN_SWITCH_AUX3b, INPUT_PULLUP);
  }

  if (DIGITAL_PIN_SWITCH_AUX4a != 0) {
    pinMode(DIGITAL_PIN_SWITCH_AUX4a, INPUT_PULLUP);
    pinMode(DIGITAL_PIN_SWITCH_AUX4b, INPUT_PULLUP);
  }
...
void loop (void)
{
  ...
  */
  ...
  // Handle reversed (upside down) joysticks
  ...
  // arm-switch (in elrs) is mandatory
  // 2pos
  ...
  // 3pos
  if (DIGITAL_PIN_SWITCH_AUX3a != 0) {
    AUX3a_value = digitalRead(DIGITAL_PIN_SWITCH_AUX3a);
    AUX3b_value = digitalRead(DIGITAL_PIN_SWITCH_AUX3b);
  }

  // 3pos
  if (DIGITAL_PIN_SWITCH_AUX4a != 0) {
    AUX4a_value = digitalRead(DIGITAL_PIN_SWITCH_AUX4a);
    AUX4b_value = digitalRead(DIGITAL_PIN_SWITCH_AUX4b);
  }
  ...
  if (DIGITAL_PIN_SWITCH_AUX3a != 0) {
    ...
    if ( (AUX3a_value == 0) && (AUX3b_value == 0) ) {
      rcChannels[AUX3] = CRSF_DIGITAL_CHANNEL_MIN;

    } else if ( (AUX3a_value == 1) && (AUX3b_value == 1) ) {
      rcChannels[AUX3] = CRSF_DIGITAL_CHANNEL_MAX;

    } else {
      rcChannels[AUX3] = (CRSF_DIGITAL_CHANNEL_MIN + CRSF_DIGITAL_CHANNEL_MAX) / 2; // midpoint
    }
  }
  ...
  if (DIGITAL_PIN_SWITCH_AUX4a != 0) {
    // 3pos
    if ( (AUX4a_value == 0) && (AUX4b_value == 0) ) {
      rcChannels[AUX4] = CRSF_DIGITAL_CHANNEL_MIN;

    } else if ( (AUX4a_value == 1) && (AUX4b_value == 1) ) {
      rcChannels[AUX4] = CRSF_DIGITAL_CHANNEL_MAX;

    } else {
      rcChannels[AUX4] = (CRSF_DIGITAL_CHANNEL_MIN + CRSF_DIGITAL_CHANNEL_MAX) / 2; // midpoint
    }
  }
